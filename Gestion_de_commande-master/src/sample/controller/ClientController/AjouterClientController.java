...
public class AjouterClientController implements Initializable {
    ...
    void AddClient(ActionEvent event) {
        ...
        nom.resetValidation();
        num_tel.resetValidation();
        if ((num_tel.getText().isEmpty() || prenom.getText().isEmpty() || email.getText().isEmpty()
                || nom.getText().isEmpty())) {
            valide = false;
            num_tel.validate();
            prenom.validate();
            ...
            if (!num_tel.getText().matches("[0-9]")) {
                num_tel.validate();
                valide = false;
            }
        ...
        if (add.getText() != "Modifier" && valide) {
            ...
            if (this.sexe.getSelectedToggle().equals(this.homme)) {
                LibeleSexe = "Homme";
            }
            if (this.sexe.getSelectedToggle().equals(this.femme)) {
                LibeleSexe = "Femme";
            }
            ...
            alert.setContentText("Client Ajouter avec réussite  ");
            alert.showAndWait();
            ...
            c.Upadate(this.li.id_client, num_tel.getText(), nom.getText(), prenom.getText(), LibeleSexe,
                    email.getText());
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setHeaderText(null);
            alert.setContentText("Client modifieé avec réussite ");
            alert.showAndWait();
        }
